...
-   X = factor ( 1 = "step 1 first", 2 = "step 2 first", 3 = "only step 1", 4 = "only step 2") - \> note only 1's and 2's exist in the cleaned data
...
exam_order_mod <- glm( formula = score_step2 ~ order_factor + school_factor ,data = step2_complete)
summary(exam_order_mod)
```

Based on the model output (p values), there doesn't appear to be any signifigant associations between exam order and the score for Step 2.
...
::: callout-important
The pairs plot needs some attention/feedback to make it useful for preseting.
:::
...
I decided to exclude the number of practice tests (covariate X4) since the responses were so few:
...
mod_step2_scores <- glm(score_step2 ~ uworld_percent_step2 + amboss_percent_step2 + length_step2 + simulate_full_practice + practice_score_step2 + school_factor, data = step2_complete)
...
"push_score_only_step1_1" not currently included but could be "push_practice_test_step1_1" yes\
"push_nbme_practice_score_step1_1" yes "push_uw_practice_score_step1_1" yes

```{r}
step1_complete$push_step1 <- ifelse(step1_complete$push_step1 == 2 | is.na(step1_complete$push_step1) ==TRUE, 2, 1) #recoding the NA's to be "No" (they did not push back step 1)
...
table1(~ push_practice_test_step1 + push_uw_practice_score_step1 + push_nbme_practice_score_step1 |push_step1, data=step1_complete, topclass="Rtable1-zebra")
