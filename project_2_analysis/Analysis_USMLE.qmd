...
dt_q3 = step1_complete %>% 
  ...
  left_join(step2_complete %>% select(record_id,schoolid, simulate_full_practice,practice_score_step2)) %>% filter(!is.na(push_step1))


dt_q3_impute = mice(dt_q3,m = 1,method = 'sample') %>% complete()

model = glm(
  push_step1~ uworld_clean + amboss_clean + numeric_length_step1 + simulate_full_practice + practicetest_step1 + schoolid,
  dt_q3_impute,
  family = 'binomial'
)
...
ggplot(aes(x = simulate_full_practice), data= dt_q3) +
  coord_flip() +
  scale_color_brewer(palette = "Greens") +
  geom_bar() +
  theme_minimal() +
