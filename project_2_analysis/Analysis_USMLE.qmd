...
## Q2 What factors affect Step 2 score? (do not report this yet until we decide)
...
cv.glmnet(my_design_mat[,-1],step2_impute$score_step2,family = 'gaussian',standardize = T,type.measure = 'mse')

model = glmnet(my_design_mat[,-1],step2_impute$score_step2,family = 'gaussian',standardize = T,lambda = 1.653 )

coef(model)

#now I need to make a model with a more limited set of predictors (this will stay the same since we set the seed)
# as.factor(length_step2)1-2 weeks  
# as.factor(length_step2)more than 8 weeks
# practice_score_step2 

#recode length step 2 to have 2 indicators
